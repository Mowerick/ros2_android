...
namespace ros2_android
{
  ...
  const char *TargetManagerStateName(TargetManagerState state)
  {
    ...
    case TargetManagerState::INIT:
      return "INIT";
    case TargetManagerState::HARDHOME:
      return "HARDHOME";
    case TargetManagerState::SETUP_PHASE:
      return "SETUP_PHASE";
    case TargetManagerState::CALIBRATING:
      return "CALIBRATING";
    case TargetManagerState::SOFTHOME:
      return "SOFTHOME";
    case TargetManagerState::READY:
      return "READY";
    case TargetManagerState::FIXED_POSITION_MODE:
      return "FIXED_POSITION_MODE";
    case TargetManagerState::SENT_CMD:
      return "SENT_CMD";
    case TargetManagerState::EXECUTING:
      return "EXECUTING";
    }
    return "UNKNOWN";
  }
  ...
  std::string TargetManagerController::GetLastMeasurementJson()
  {
    ...
    ss << "{\"state\":\"" << TargetManagerStateName(state_) << "\""
       << ",\"esp32_state\":" << (latest_feedback_.has_value() ? static_cast<int>(latest_feedback_->state) : -1)
      ...
      const std_msgs::msg::Float32MultiArray::SharedPtr msg)
  {
    ...
    int32_t cur_tilt_r1 = (latest_feedback_->resolution > 0)
                              ? latest_feedback_->current_steps[0] / static_cast<int32_t>(latest_feedback_->resolution)
                              : 0;
    int32_t cur_pan_r1 = (latest_feedback_->resolution > 0)
                             ? latest_feedback_->current_steps[1] / static_cast<int32_t>(latest_feedback_->resolution)
                             : 0;
      ...
      const geometry_msgs::msg::Point &target, const LatestFeedback &fb)
  {
    ...
    int32_t cur_tilt_r1 = (fb.resolution > 0)
                              ? fb.current_steps[0] / static_cast<int32_t>(fb.resolution)
                              : 0;
    int32_t cur_roll_r1 = (fb.resolution > 0)
                              ? fb.current_steps[1] / static_cast<int32_t>(fb.resolution)
                              : 0;
    int32_t delta = std::max(std::abs(tilt_at_r1 - cur_tilt_r1),
                             std::abs(roll_at_r1 - cur_roll_r1));
    uint8_t res = SelectResolution(delta, kThresholdCoarse, kThresholdFine,
  ...
  void TargetManagerController::TickSetupPhase()
  {
    ...
    {
      LOGD("SetupPhase: re-issuing SETUP");
      SendCurrentSetup();
      return;
    }
    if (SetupEchoed())
  ...
  void TargetManagerController::TickCalibrating()
  {
    if (!latest_feedback_.has_value() ||
        ...
        !imu_quat_.has_value())
    {
      LOGD("Calibration: Not ready. feedback_has_value: %d, feedback_state: %d, imu_has_value: %d",
           latest_feedback_.has_value(),
           latest_feedback_.has_value() ? latest_feedback_->state : -1,
           imu_quat_.has_value());
      return;
    }
